...
APP_PLAYLIST_DESCRIPTION_MARKER = "Generated by Spotify Playlist Generator"
...
def canonicalize_genre_name(raw_genre):
    normalized_genre = normalize_text(raw_genre).replace("-", " ")
    if normalized_genre == "k pop":
        return "k-pop"
    if normalized_genre == "lo fi":
        return "lofi"
    return normalized_genre


def parse_generated_playlist_description(description):
    description_text = str(description or "").strip()
    if APP_PLAYLIST_DESCRIPTION_MARKER not in description_text:
        return None

    match = re.match(
        r"^(?P<mood>.+?) mood, (?P<genre>.+?) direction, (?P<vibe>.+?) finish\.",
        description_text,
        flags=re.IGNORECASE,
    )
    if not match:
    ...
    return {
        "mood": normalize_text(match.group("mood")),
        "genre": canonicalize_genre_name(match.group("genre")),
        "vibe": normalize_text(match.group("vibe")),
    }


def sync_generated_playlists_from_spotify(sp, user_id, page_limit=50):
    synced_count = 0
    offset = 0

    while True:
        response = sp.current_user_playlists(limit=page_limit, offset=offset)
        ...
        for playlist in items:
            ...
            owner_id = ((playlist.get("owner") or {}).get("id") or "").strip()
            if owner_id != user_id:
                continue

            playlist_metadata = parse_generated_playlist_description(
                playlist.get("description")
            )
            if not playlist_metadata:
                continue

            playlist_url = ((playlist.get("external_urls") or {}).get("spotify") or "").strip()
            playlist_name = str(playlist.get("name") or "").strip()
            if not playlist_url or not playlist_name:
                continue

            already_exists = playlists_collection.count_documents(
                {
                    "user_id": user_id,
            ...
            history_payload = {
                "user_id": user_id,
                "mood": playlist_metadata["mood"],
                "genre": playlist_metadata["genre"],
                "vibe": playlist_metadata["vibe"],
                "count": track_count,
...
def playlist_history():
    ...
    try:
        ...
        sync_generated_playlists_from_spotify(sp, user["id"])
        history_items = list(
